...
bool H263Container::H263StartEncoder(int w, int h, int fps)
{
	...
	if (!h263Encoder)
	{
		kdDebug() << "Could not find H.263 Encoder" << endl;
		return false;
	}
	...
	h263EncContext->bit_rate = 20000000;
	...
	{
		kdDebug() << "Could not open H.263 Encoder" << endl;
		return false;
	}
...
bool H263Container::H263StartDecoder(int w, int h)
{
	...
	if (!h263Decoder)
	{
		kdDebug() << "Could not find H.263 decoder" << endl;
		return false;
	}
	...
	{
		kdDebug() << "Could not open H.263 Decoder" << endl;
		return false;
	}
...
uchar *H263Container::H263DecodeFrame(const uchar *h263Frame, int h263FrameLen, uchar *rgbBuffer, int rgbBufferSize)
{
	...
	if (len != h263FrameLen)
	{
		kdDebug() << "Error decoding frame; " << len << endl;
		return 0;
	}
...
void YUV420PtoRGB32(int width, int height, int stride, const unsigned char *src, unsigned char *dst, int dstSize)
{
	...
	{
		kdDebug() << "YUVtoRGB buffer (" << dstSize << ") too small for " << width << "x" << height << " pixels" << endl;
		return;
	}
...
void cropYuvImage(const uchar *yuvBuffer, int ow, int oh, int cx, int cy, int cw, int ch, uchar *dst)
{
	...
	{
		kdDebug() << "YUV crop fn does not handle odd sizes; x,y=" << cx << "," << cy << "  w,h=" << cw << "," << ch << endl;
		return;
	}
